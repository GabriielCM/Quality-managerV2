...
export default function ConsertoCreatePage() {
  ...
  const loadRncsAceitas = async () => {
    try {
      ...
      // Filtrar apenas RNCs que ainda não possuem devolução ou conserto vinculados
      const rncsDisponiveis = data.filter((rnc) => !rnc.devolucao && !rnc.conserto);
      setRncsAceitas(rncsDisponiveis);
    } catch (error) {
